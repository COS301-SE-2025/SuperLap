...
public static class DrivingRecommendationEngine
{
    public static void UpdateDrivingRecommendations(bool enableRecommendations, Vector3 currentPosition, Vector3 currentForward, 
                                                   ...
                                                   out bool recommendTurnLeft, out bool recommendTurnRight)
    {
        ...
        List<Vector2> optimalRaceline = RacelineAnalyzer.GetOptimalRaceline();
        ...
        RecommendationAnalysis analysis = AnalyzeRecommendations(optimalRaceline, currentPosition, currentForward, 
                                                               ...
                                                               theoreticalTopSpeed, config, physicsConfig, trackConfig);
    ...
    private static RecommendationAnalysis AnalyzeRecommendations(List<Vector2> raceline, Vector3 currentPosition, 
                                                               Vector3 currentForward, float currentSpeed, 
                                                               ...
                                                               MotorcyclePhysicsConfig physicsConfig, TrackDetectionConfig trackConfig)
    {
        ...
        float offTrackRatio;
        bool willGoOffTrack = TrackDetector.CheckIfPathGoesOffTrack(optimalSteeringInput, currentPosition, currentForward, 
                                                                   currentSpeed, currentTurnAngle, throttleInput, 
                                                                   config.trajectoryLength, config.recommendationSteps, 
                                                                   config.offTrackThreshold, physicsConfig, trackConfig, 
                                                                   out offTrackRatio);
    ...
    private static float SimulateRecommendationScenario(float testThrottle, float testSteering, List<Vector2> raceline, 
                                                      Vector3 currentPosition, Vector3 currentForward, float currentSpeed, 
                                                      ...
                                                      MotorcyclePhysicsConfig physicsConfig)
    {
        ...
        for (int step = 0; step < config.recommendationSteps; step++)
        {
            ...
            float deviation = RacelineAnalyzer.CalculateDistanceToRaceline(simPos2D, raceline);
