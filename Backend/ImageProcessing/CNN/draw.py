...
def draw_boundaries_matplotlib(outer_boundary, inner_boundary, title="Track Boundaries"):
    ...
    if outer_boundary:
        ...
        plt.plot(outer_closed[:, 0], outer_closed[:, 1], 'r-', linewidth=2, label='Outer Boundary')
    ...
    if inner_boundary:
        ...
        plt.plot(inner_closed[:, 0], inner_closed[:, 1], 'b-', linewidth=2, label='Inner Boundary')
...
def save_boundary_image(outer_boundary, inner_boundary, output_path, image_size=(800, 600)):
...
def print_boundary_stats(outer_boundary, inner_boundary):
    ...
    if outer_boundary:
        ...
        print(f"Outer boundary X range: {outer_points[:, 0].min():.1f} to {outer_points[:, 0].max():.1f}")
        print(f"Outer boundary Y range: {outer_points[:, 1].min():.1f} to {outer_points[:, 1].max():.1f}")
    else:
    ...
    if inner_boundary:
        ...
        print(f"Inner boundary X range: {inner_points[:, 0].min():.1f} to {inner_points[:, 0].max():.1f}")
        print(f"Inner boundary Y range: {inner_points[:, 1].min():.1f} to {inner_points[:, 1].max():.1f}")
    else:
...
def main():
    ...
    for img_path in possible_images:
        if os.path.exists(img_path):
            ...
            overlay_success = draw_boundaries_on_image(outer_boundary, inner_boundary, img_path)
            break
    
    if not overlay_success:
        print(f"\nNo original image found. Tried: {', '.join(possible_images)}")
        print("To overlay on original image, ensure one of these files exists in the current directory")
    ...
    if all_points:
        ...
        save_boundary_image(outer_boundary, inner_boundary, output_image, (width, height))
