...
def geojson_to_labelstudio(track_name, geojson_path):
    ...
    for feat in gj.get("features", []):
        ...
        if isinstance(geom, Polygon):
            priority = (
                0 if any(keyword in label for keyword in ["outer", "outside"]) else 1
            )
    ...
    for priority, feat in features_sorted:
        ...
        if isinstance(geom, Polygon):
            ...
            boxes.append({"label": final_label, "points": outer_scaled})

            for hole in geom.interiors:
                inner_coords = list(map(list, hole.coords))
                inner_scaled = [point_map[(pt[0], pt[1])] for pt in inner_coords]

                boxes.append({"label": "inner", "points": inner_scaled})
    data = {"width": int(width), "height": int(height), "boxes": boxes}
