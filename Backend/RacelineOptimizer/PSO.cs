...
namespace RacelineOptimizer
{
  public class PSO
  {
    ...
    private static bool IsTrackCounterClockwise(List<Vector2> trackPoints)
    {
      float area = 0f;
      for (int i = 0; i < trackPoints.Count; i++)
      {
        var p1 = trackPoints[i];
        var p2 = trackPoints[(i + 1) % trackPoints.Count];
        area += (p2.X - p1.X) * (p2.Y + p1.Y);
      }
      return area < 0f;
    }

    private static float GetCornerBias(List<CornerDetector.CornerSegment> corners, int index, int currentTrackLength, int cornerTrackLength, bool isCounterClockwise)
    {
        ...
        foreach (var corner in corners)
        {
            ...
            bool effectiveIsLeft = isCounterClockwise ? corner.IsLeftTurn : !corner.IsLeftTurn;

            return effectiveIsLeft
                ? 0.5f - biasOffset
                : 0.5f + biasOffset;
        }
