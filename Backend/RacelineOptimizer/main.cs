    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (!int.TryParse(Console.ReadLine(), out int ch) || ch == 1)
            {
                ...
                if (choice == csvFiles.Length + 1)
                {
                    foreach (string filePath in csvFiles)
                    {
                        ...
                        ConvertCSVToBin(filePath, outputPath);
                    }
                }
                else
                {
                    ...
                    ConvertCSVToBin(selectedFile, outputPath);
                }
                return;
            }
            else
            {
                ...
                if (!Directory.Exists(inputDir))
                {
                    Console.WriteLine("Input directory does not exist. Please create it and add .bin files.");
                    return;
                }
        ...
        public static void ConvertCSVToBin(string csvPath, string binOutputPath, int targetLapIndex = 0)
        {

            RacelineVisualizer.LoadCSV.Convert(csvPath, binOutputPath, targetLapIndex);
        }
    }
}
